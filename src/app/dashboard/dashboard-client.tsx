...
export function DashboardClient() {
  ...
  return (
    <div className="space-y-8 pb-8">
      ...
      {metrics.dueToday && metrics.dueToday.length > 0 ? (
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-sm font-bold text-slate-900">Today's Collectibles</h3>
            <p className="mt-0.5 text-xs text-slate-500">
              {metrics.dueTodayCount} accounts &middot; {formatPeso(metrics.dueTodayTotal)} total due
            </p>
          </div>
          <Link href="/loans" className="text-xs font-medium text-red-700 hover:text-red-800 transition-colors">
            View All &rarr;
          </Link>
        </div>
        <div className="divide-y divide-slate-100">
          {metrics.dueToday.map((item) => (
            <Link
              key={`${item.loanAccountId}-${item.periodNumber}`}
              href={`/loans/${item.loanAccountId}`}
            ...
            >
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-slate-500 truncate">{item.customerAddress}</p>
              </div>
              <div className="text-right flex-shrink-0">
                <p className="text-sm font-bold text-slate-900">{formatPeso(item.amount)}</p>
                <span className={`inline-block mt-0.5 text-[10px] font-semibold uppercase px-1.5 py-0.5 rounded-full ${
                  item.status === "OVERDUE"
                  ...
                  {item.status === "OVERDUE" ? "Overdue" : item.status === "PARTIAL" ? "Partial" : "Due"}
                </span>
              </div>
            </Link>
          ))}
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-sm font-bold text-slate-900">Today's Collectibles</h3>
            <p className="mt-0.5 text-xs text-slate-500">No accounts due today &mdash; all caught up!</p>
          </div>
          <Link href="/loans" className="text-xs font-medium text-red-700 hover:text-red-800 transition-colors">
            View All Loans &rarr;
          </Link>
